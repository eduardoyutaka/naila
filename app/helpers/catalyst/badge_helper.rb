module Catalyst::BadgeHelper
  ...
  BADGE_BASE = "inline-flex items-center gap-x-1.5 rounded-md px-1.5 py-0.5 text-sm/5 font-medium sm:text-xs/5 forced-colors:outline".freeze

  # Renders a Catalyst badge.
  #   catalyst_badge("Active", color: "green")
  def catalyst_badge(text = nil, color: "zinc", **opts, &block)
    content = block ? capture(&block) : text
    css = [BADGE_BASE, BADGE_COLORS.fetch(color.to_s, BADGE_COLORS["zinc"]), opts.delete(:class)].compact.join(" ")
