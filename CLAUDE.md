...
- An individual `Alarm` can further narrow to a *single* one of its basin's configured stations, via `Alarm#monitoring_station` (belongs_to, optional) — managed by the alarm form's "Estação" dropdown. `Alarm#effective_monitoring_stations` is what evaluation actually uses: `[monitoring_station]` if set, otherwise every station configured for the basin. Deliberately single, not multi-select — blending multiple physically distinct rain gauges via `Sum` isn't a meaningful accumulated-rainfall value, and the real use case (different thresholds per station) means separate alarms, not one alarm reading several stations at once. The scoped station must already be one of the basin's configured stations — validated on the alarm, not just enforced by the UI.
