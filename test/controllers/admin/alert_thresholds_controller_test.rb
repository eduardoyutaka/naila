require "test_helper"

class Admin::AlertThresholdsControllerTest < ActionDispatch::IntegrationTest
  setup do
    sign_in_as users(:admin)
    @alert = alerts(:active_high)
  end

  # ── Authentication ──

  test "new requires authentication" do
    reset!
    get new_admin_alert_threshold_path(alert_id: @alert.id)
    assert_redirected_to login_path
  end

  # ── New ──

  test "new renders successfully" do
    get new_admin_alert_threshold_path(alert_id: @alert.id)
    assert_response :success
  end

  test "new displays form fields" do
    get new_admin_alert_threshold_path(alert_id: @alert.id)
    assert_select "form" do
      assert_select "select[name='alert_threshold[parameter]']"
      assert_select "select[name='alert_threshold[threshold_type]']"
      assert_select "input[name='alert_threshold[value]']"
      assert_select "select[name='alert_threshold[unit]']"
      assert_select "select[name='alert_threshold[comparison]']"
      assert_select "select[name='alert_threshold[severity]']"
    end
  end

  test "new pre-fills river_basin_id and river_id from params" do
    get new_admin_alert_threshold_path(
      alert_id: @alert.id,
      river_basin_id: @alert.river_basin_id,
      river_id: @alert.river_id
    )
    assert_response :success
  end

  test "new includes back link to alert" do
    get new_admin_alert_threshold_path(alert_id: @alert.id)
    assert_select "a[href='#{admin_alert_path(@alert)}']", text: /Voltar ao alerta/
  end

  # ── Create ──

  test "create with valid params creates threshold and redirects to alert" do
    assert_difference "AlertThreshold.count", 1 do
      post admin_alert_thresholds_path, params: {
        alert_id: @alert.id,
        alert_threshold: {
          parameter: "river_level",
          threshold_type: "instant",
          severity: 2,
          value: 2.0,
          unit: "m",
          comparison: "gte",
          cooldown_minutes: 60
        }
      }
    end
    assert_redirected_to admin_alert_path(@alert)
  end

  test "create without alert_id redirects to alerts index" do
    assert_difference "AlertThreshold.count", 1 do
      post admin_alert_thresholds_path, params: {
        alert_threshold: {
          parameter: "river_level",
          threshold_type: "instant",
          severity: 2,
          value: 2.0,
          unit: "m",
          comparison: "gte",
          cooldown_minutes: 60
        }
      }
    end
    assert_redirected_to admin_alerts_path
  end

  test "create with invalid params renders new with 422" do
    assert_no_difference "AlertThreshold.count" do
      post admin_alert_thresholds_path, params: {
        alert_id: @alert.id,
        alert_threshold: { parameter: "", value: nil, severity: nil }
      }
    end
    assert_response :unprocessable_entity
  end

  test "operator cannot create thresholds" do
    sign_in_as users(:operator)
    post admin_alert_thresholds_path, params: {
      alert_id: @alert.id,
      alert_threshold: {
        parameter: "river_level",
        threshold_type: "instant",
        severity: 2,
        value: 2.0,
        unit: "m",
        comparison: "gte",
        cooldown_minutes: 60
      }
    }
    assert_redirected_to admin_root_path
  end

  # ── Edit ──

  test "edit renders successfully" do
    get edit_admin_alert_threshold_path(alert_thresholds(:precipitation_high), alert_id: @alert.id)
    assert_response :success
  end

  test "edit displays form with existing values" do
    threshold = alert_thresholds(:precipitation_high)
    get edit_admin_alert_threshold_path(threshold, alert_id: @alert.id)
    assert_select "input[name='alert_threshold[value]'][value='#{threshold.value}']"
  end

  test "edit includes back link to alert" do
    get edit_admin_alert_threshold_path(alert_thresholds(:precipitation_high), alert_id: @alert.id)
    assert_select "a[href='#{admin_alert_path(@alert)}']", text: /Voltar ao alerta/
  end

  # ── Update ──

  test "update with valid params saves and redirects to alert" do
    threshold = alert_thresholds(:precipitation_high)
    patch admin_alert_threshold_path(threshold), params: {
      alert_id: @alert.id,
      alert_threshold: { value: 75.0 }
    }
    assert_redirected_to admin_alert_path(@alert)
    assert_equal 75.0, threshold.reload.value
  end

  test "update with invalid params renders edit with 422" do
    threshold = alert_thresholds(:precipitation_high)
    patch admin_alert_threshold_path(threshold), params: {
      alert_id: @alert.id,
      alert_threshold: { severity: 99 }
    }
    assert_response :unprocessable_entity
  end

  test "operator cannot update thresholds" do
    sign_in_as users(:operator)
    patch admin_alert_threshold_path(alert_thresholds(:precipitation_high)), params: {
      alert_id: @alert.id,
      alert_threshold: { value: 99.0 }
    }
    assert_redirected_to admin_root_path
  end

  # ── Destroy ──

  test "admin can destroy threshold and redirects to alert" do
    threshold = alert_thresholds(:precipitation_high)
    assert_difference "AlertThreshold.count", -1 do
      delete admin_alert_threshold_path(threshold), params: { alert_id: @alert.id }
    end
    assert_redirected_to admin_alert_path(@alert)
  end

  test "destroy without alert_id redirects to alerts index" do
    threshold = alert_thresholds(:precipitation_high)
    assert_difference "AlertThreshold.count", -1 do
      delete admin_alert_threshold_path(threshold)
    end
    assert_redirected_to admin_alerts_path
  end

  test "coordinator cannot destroy threshold" do
    sign_in_as users(:coordinator)
    delete admin_alert_threshold_path(alert_thresholds(:precipitation_high)), params: { alert_id: @alert.id }
    assert_redirected_to admin_root_path
  end

  test "operator cannot destroy threshold" do
    sign_in_as users(:operator)
    delete admin_alert_threshold_path(alert_thresholds(:precipitation_high)), params: { alert_id: @alert.id }
    assert_redirected_to admin_root_path
  end
end
